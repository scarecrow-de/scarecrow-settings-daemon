...
static void     csd_power_manager_class_init  (GsdPowerManagerClass *klass);
static void     csd_power_manager_init        (GsdPowerManager      *power_manager);
...
G_DEFINE_TYPE (GsdPowerManager, csd_power_manager, G_TYPE_OBJECT)
                      ...
                      guint            logout_mode)
{
        if (g_getenv ("RUNNING_UNDER_GDM")) {
                g_warning ("Prevented logout from GDM session! This indicates an issue in csd-power.");
                return;
        }
...
backlight_notify_brightness_cb (GsdPowerManager *manager, GParamSpec *pspec, GsdBacklight *backlight)
{
        backlight_iface_emit_changed (manager, GSD_POWER_DBUS_INTERFACE_SCREEN,
                                      csd_backlight_get_brightness (backlight, NULL), NULL);
}
                       ...
                       gint idle_percentage)
{
        ...
        csd_backlight_get_brightness (manager->backlight, &brightness);
        if (brightness < idle_percentage)
                return;

        manager->pre_dim_brightness = brightness;
        csd_backlight_set_brightness_async (manager->backlight, idle_percentage, NULL, NULL, NULL);
}
...
idle_set_mode (GsdPowerManager *manager, GsdPowerIdleMode mode)
{
        ...
        /* turn on screen and restore user-selected brightness level */
                ...
                if (manager->backlight && manager->pre_dim_brightness >= 0) {
                        csd_backlight_set_brightness_async (manager->backlight,
...
csd_power_manager_finalize (GObject *object)
{
        ...
        csd_power_manager_stop (manager);
...
static void
csd_power_manager_class_init (GsdPowerManagerClass *klass)
{
                       ...
                       gpointer      user_data)
{
        ...
        manager->backlight = csd_backlight_new (manager->rr_screen, NULL);
        ...
        if (!gnome_settings_is_wayland ())
                manager->xscreensaver_watchdog_timer_id = csd_power_enable_screensaver_watchdog ();

        /* don't blank inside a VM */
        manager->is_virtual_machine = csd_power_is_hardware_a_vm ();
        ...
        */
        if (manager->backlight) {
                manager->ambient_percentage_old = csd_backlight_get_brightness (manager->backlight, NULL);
                backlight_iface_emit_changed (manager, GSD_POWER_DBUS_INTERFACE_SCREEN,
...
iio_proxy_changed (GsdPowerManager *manager)
{
        ...
        if (manager->backlight)
                csd_backlight_set_brightness_async (manager->backlight, pc, NULL, NULL, NULL);
...
gboolean
csd_power_manager_start (GsdPowerManager *manager,
                         GError **error)
{
...
void
csd_power_manager_stop (GsdPowerManager *manager)
{
...
static void
csd_power_manager_init (GsdPowerManager *manager)
{
        manager->inhibit_lid_switch_fd = -1;
        manager->inhibit_suspend_fd = -1;
                              ...
                              gpointer user_data)
{
        ...
        manager = g_object_get_data (G_OBJECT (invocation), "csd-power-manager");
        brightness = csd_backlight_set_brightness_finish (backlight, res, &error);
        ...
        if (error) {
                ...
                connector = csd_backlight_get_connector (backlight);
                             ...
                             gpointer user_data)
{
        ...
        brightness = csd_backlight_set_brightness_finish (backlight, res, NULL);
                           ...
                           GDBusMethodInvocation *invocation)
{
        ...
        g_object_set_data_full (G_OBJECT (invocation), "csd-power-manager", g_object_ref (manager), g_object_unref);

        if (g_strcmp0 (method_name, "StepUp") == 0) {
                g_debug ("screen step up");
                csd_backlight_step_up_async (manager->backlight, NULL, backlight_brightness_step_cb, invocation);

        } else if (g_strcmp0 (method_name, "StepDown") == 0) {
                g_debug ("screen step down");
                csd_backlight_step_down_async (manager->backlight, NULL, backlight_brightness_step_cb, invocation);

        } else if (g_strcmp0 (method_name, "Cycle") == 0) {
                g_debug ("screen cycle up");
                csd_backlight_cycle_up_async (manager->backlight, NULL, backlight_brightness_step_cb, invocation);
                           ...
                           GError **error)
{
        ...
        if (g_strcmp0 (interface_name, GSD_POWER_DBUS_INTERFACE_SCREEN) == 0) {
                if (manager->backlight)
                        value = csd_backlight_get_brightness (manager->backlight, NULL);
                else
                        value = -1;
                           ...
                           GError **error)
{
        ...
        if (g_strcmp0 (interface_name, GSD_POWER_DBUS_INTERFACE_SCREEN) == 0) {
                ...
                if (manager->backlight) {
                        csd_backlight_set_brightness_async (manager->backlight, brightness_value,
                                                            NULL,
