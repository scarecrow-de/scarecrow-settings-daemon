...
#include "csd-remote-display-manager.h"
...
#define GSD_XSETTINGS_ERROR csd_xsettings_error_quark ()

enum {
        GSD_XSETTINGS_ERROR_INIT
};

static void     csd_xsettings_manager_class_init  (GsdXSettingsManagerClass *klass);
static void     csd_xsettings_manager_init        (GsdXSettingsManager      *xsettings_manager);
static void     csd_xsettings_manager_finalize    (GObject                  *object);

static void     register_manager_dbus             (GsdXSettingsManager *manager);

G_DEFINE_TYPE (GsdXSettingsManager, csd_xsettings_manager, G_TYPE_OBJECT)
                 ...
                 GtkSettingsMask      mask)
{
        ...
        if (mask & GTK_SETTINGS_MODULES) {
                const char *modules = csd_xsettings_gtk_get_modules (manager->gtk);
...
gboolean
csd_xsettings_manager_start (GsdXSettingsManager *manager,
                             GError             **error)
{
        GVariant    *overrides;
        guint        i;
        ...
        manager->gtk = csd_xsettings_gtk_new ();
...
void
csd_xsettings_manager_stop (GsdXSettingsManager *manager)
{
...
static void
csd_xsettings_manager_class_init (GsdXSettingsManagerClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        object_class->finalize = csd_xsettings_manager_finalize;
}

static void
csd_xsettings_manager_init (GsdXSettingsManager *manager)
{
        GError *error = NULL;
...
static void
csd_xsettings_manager_finalize (GObject *object)
{
        ...
        csd_xsettings_manager_stop (xsettings_manager);
        ...
        G_OBJECT_CLASS (csd_xsettings_manager_parent_class)->finalize (object);
}
...
GsdXSettingsManager *
csd_xsettings_manager_new (void)
{
