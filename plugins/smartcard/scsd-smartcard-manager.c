...
static void     csd_smartcard_manager_class_init  (GsdSmartcardManagerClass *klass);
static void     csd_smartcard_manager_init        (GsdSmartcardManager      *self);
...
static void     on_smartcards_from_driver_watched (GsdSmartcardManager *self,
                                                   GAsyncResult        *result,
                                                   GTask               *task);
G_DEFINE_TYPE (GsdSmartcardManager, csd_smartcard_manager, G_TYPE_OBJECT)
G_DEFINE_QUARK (csd-smartcard-manager-error, csd_smartcard_manager_error)
G_LOCK_DEFINE_STATIC (csd_smartcards_watch_tasks);
...
static void
csd_smartcard_manager_class_init (GsdSmartcardManagerClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->finalize = csd_smartcard_manager_finalize;

        csd_smartcard_utils_register_error_domain (GSD_SMARTCARD_MANAGER_ERROR,
                                                   GSD_TYPE_SMARTCARD_MANAGER_ERROR);
}

static void
csd_smartcard_manager_init (GsdSmartcardManager *self)
{
}
                             ...
                             GError                   **error)
{
        ...
        if (old_card != NULL) {
                ...
                if (old_slot_series != slot_series) {
                         ...
                         * removal
                         */
                        csd_smartcard_service_sync_token (self->service, old_card, cancellable);
                }
        ...
        if (PK11_IsPresent (card)) {
                ...
                csd_smartcard_service_sync_token (self->service, card, cancellable);
...
on_main_thread_to_register_driver (GTask *task)
{
        ...
        csd_smartcard_service_register_driver (self->service,
                                               operation->driver);
                          ...
                          GTask               *task)
{
        ...
        login_token = csd_smartcard_manager_get_login_token (self);
        ...
        if (login_token || g_getenv ("PKCS11_LOGIN_TOKEN_NAME") != NULL) {
                ...
                if (!login_token || !PK11_IsPresent (login_token))
                        csd_smartcard_manager_do_remove_action (self);
        }
                    ...
                    GsdSmartcardManager *self)
{
        ...
        service = csd_smartcard_service_new_finish (result, &error);
...
static gboolean
csd_smartcard_manager_idle_cb (GsdSmartcardManager *self)
{
        ...
        csd_smartcard_service_new_async (self,
                                         self->cancellable,
...
gboolean
csd_smartcard_manager_start (GsdSmartcardManager  *self,
                             GError              **error)
{
        gnome_settings_profile_start (NULL);

        self->start_idle_id = g_idle_add ((GSourceFunc) csd_smartcard_manager_idle_cb, self);
        g_source_set_name_by_id (self->start_idle_id, "[scarecrow-settings-daemon] csd_smartcard_manager_idle_cb");
...
void
csd_smartcard_manager_stop (GsdSmartcardManager *self)
{
...
lock_screen (GsdSmartcardManager *self)
{
        ...
        csd_screen_saver_call_lock (self->screen_saver,
                                    self->cancellable,
               ...
               GsdSmartcardManager *self)
{
        ...
        is_logged_out = csd_session_manager_call_logout_finish (session_manager, result, &error);
...
log_out (GsdSmartcardManager *self)
{
        ...
        csd_session_manager_call_logout (self->session_manager,
...
void
csd_smartcard_manager_do_remove_action (GsdSmartcardManager *self)
{
        char *remove_action;
...
PK11SlotInfo *
csd_smartcard_manager_get_login_token (GsdSmartcardManager *self)
{
...
GList *
csd_smartcard_manager_get_inserted_tokens (GsdSmartcardManager *self,
                                           gsize               *num_tokens)
{
...
csd_smartcard_manager_finalize (GObject *object)
{
        ...
        csd_smartcard_manager_stop (self);

        G_OBJECT_CLASS (csd_smartcard_manager_parent_class)->finalize (object);
}

GsdSmartcardManager *
csd_smartcard_manager_new (void)
{
